...
void XGpioPs_StubHandler(void *CallBackRef, u32 Bank, u32 Status); /**< Stub handler */
...
* XGpioPs_StubHandlers are assigned to the Bank Status Handlers.
*
...
#endif
	InstancePtr->Handler = (XGpioPs_Handler)XGpioPs_StubHandler;
...
s32 XGpioPs_Initialize(XGpioPs *InstancePtr, u16 deviceId)
{
	s32 Status = (s32)0;
	u8 i;
	Xil_AssertNonvoid(InstancePtr != NULL);
	
	XGpioPs_Config* ConfigPtr = XGpioPs_LookupConfig(deviceId);
	Xil_AssertNonvoid(ConfigPtr != NULL);
	Xil_AssertNonvoid(ConfigPtr->DeviceId == deviceId);
	 ...
	 */
	InstancePtr->IsReady = 0U;
	InstancePtr->GpioConfig.BaseAddr = ConfigPtr->BaseAddr;
#ifndef SDT
	InstancePtr->GpioConfig.DeviceId = ConfigPtr->DeviceId;
#endif
	InstancePtr->Handler = (XGpioPs_Handler)XGpioPs_StubHandler;
	...
	// Load the interrupt mask from the register
	for (i=(u8)0U;i<InstancePtr->MaxBanks;i++) {
		u32 intMask;
                ...
                if (InstancePtr->Platform == XPLAT_VERSAL){
                        if(InstancePtr->PmcGpio == (u32)FALSE)
                        {
                                ...
                                intMask = ~(XGpioPs_ReadReg(InstancePtr->GpioConfig.BaseAddr,
                                                ((u32)(i) * XGPIOPS_REG_MASK_OFFSET) +
		...
		InstancePtr->CoreIntrMask[i] = intMask;
	}
...
*
******************************************************************************/
void XGpioPs_StubHandler(void *CallBackRef, u32 Bank, u32 Status)
{
	(void) CallBackRef;
	(void) Bank;
	(void) Status;
